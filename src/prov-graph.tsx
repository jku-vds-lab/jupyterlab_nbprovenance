...
import { addToolbarButtonClass, ReactWidget, Toolbar, ToolbarButton } from "@jupyterlab/apputils";
...
export class ProvGraph extends Panel {
    ...
    constructor() {
        ...
        for (const e in EventType) {
            const button = ReactWidget.create(React.createElement(Button, {
                ...
                <svg height="22px" width="22px" ><g transform="translate(11, 11)">{this.eventConfig[e].backboneGlyph}</g></svg>
            )));
    ...
    private updateUndoRedo() {
        this.undoButton.node.firstElementChild?.toggleAttribute("disabled", isChildNode(this.prov.current) ? false : true);
        this.redoButton.node.firstElementChild?.toggleAttribute("disabled", this.prov.current.children.length === 0 ? true : false);
    }
