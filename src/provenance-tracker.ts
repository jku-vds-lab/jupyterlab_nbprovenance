...
import { IApplicationState, EventType, NotebookProvenance } from "./notebook-provenance";
import { ICellModel, Cell } from "@jupyterlab/cells";
import { IObservableList } from "@jupyterlab/observables";
import { NotebookUtil } from "./notebook-util";
import { ActionFunction, ActionObject, ActionType, createAction } from "@visdesignlab/trrack";
...
export class NotebookProvenanceTracker {
  ...
  registerCellListeners() {
    toArray(this.notebookProvenance.notebook.model!.cells.iter()).forEach(cell => this.trackCellContentChanged(cell));
  }
  ...
  applyCellValueChange() {
    if (this.cellValueChange && this.notebookProvenance.prov.state.model.cells[this.cellValueChange.cellIndex].source !== this.cellValueChange.value) {
      this.applyAction(this.changeCellValueAction(this.cellValueChange.cellIndex, this.cellValueChange.value));
    }
  ...
  trackCellExecution() {
    NotebookActions.executed.connect((_dummy, obj: { notebook: Notebook, cell: Cell }) => {
      ...
      let index = NotebookUtil.getCellIndex(obj.notebook, obj.cell.model);
  ...
  trackCellsChanged() {
    const cellsChangedListener = (
      ...
      switch (change.type) {
        ...
        case "remove":
          this.applyAction(this.removeCellAction(change.oldIndex, notebook)); break;
        case "move":
  ...
  setupAction<Args extends unknown[] = unknown[]>(
    ...
    eventType: EventType,
    setActionProperties: (actionObject: ActionObject<IApplicationState, EventType, Args>, ...args: Args) => ActionObject<IApplicationState, EventType, Args>,
    ...
    return (...args: Args) => setActionProperties(action, ...args).call(this, ...args);
  }
   ...
   */
  applyAction(actionObject: ActionObject<IApplicationState, EventType, any>) {
  ...
  addCellAction = this.setupAction<[number, Notebook]>(
    ...
    EventType.addCell,
    (action, index, notebook) => action
      .setLabel("Cell added")
      .setMetaData({
        changedCellId: index,
        cellPositions: new Array<number>(notebook.model!.cells.length - 1).fill(0).map((v, i) => i < index ? i : i + 1)
      })
  );
  ...
  removeCellAction = this.setupAction<[number, Notebook]>(
    (state, index, notebook) => {
      ...
      if (state.activeCell !== newIndex) { state.activeCell = newIndex; }
      return state;
    },
    EventType.removeCell,
    (action, index, notebook) => {
      let cellPos = new Array<number>(notebook.model!.cells.length).fill(0).map((v, i) => i);
  ...
  moveCellAction = this.setupAction<[number, number, Notebook]>(
    (state, newIndex, oldIndex, notebook) => {
      state.model.cells.splice(newIndex, 0, state.model.cells.splice(oldIndex, 1)[0]);
